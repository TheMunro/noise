...
public class PerlinNoise : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        var range = Enumerable.Range(1, _resolution).ToList();
        Shuffle(range);
    ...
    {
        //this algorithm is non-deterministic, consider replacing with deterministic shuffle for deterministic noise
        var n = array.Count;
        for (var i = 0; i < n; i++)
        {
    ...
    {
        var minimumX = FastFloor(x);
        var minimumY = FastFloor(y);
        var minimumZ = FastFloor(z);
        ...
        minimumX = minimumX & _resolution;
        minimumY = minimumY & _resolution;
        minimumZ = minimumZ & _resolution;
        ...
        var u = SmoothStep(deltaX);
        var v = SmoothStep(deltaY);
        var w = SmoothStep(deltaZ);
